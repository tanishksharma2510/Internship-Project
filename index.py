...
The deployed app. Loads the artifacts produced by train_model.py
(artifacts.pkl) instead of retraining on every request/cold start.
Exposes `server` (the underlying Flask app) for Vercel's Python runtime.
"""
...
# ---------------------------------------------------------------------------
# Load pre-trained artifacts (no training happens here)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Static figures that only depend on training results (cheap to rebuild)
# ---------------------------------------------------------------------------
...
server = app.server  # <-- Vercel's Python runtime serves this WSGI app
...
def get_tab(tab_val):
    ...
    if tab_val == 'tab-2':
        return html.Div(children=[
            html.H2("Prediction of Tour Cancellations", style={'textAlign': 'center', 'color': 'black',
                     'font-size': 30, 'font-family': 'Times New Roman'}),
            ...
            html.Div(children=[
                ...
                html.Div(children=[
                    ...
                    dcc.Dropdown(
                        ...
                        placeholder='Select from the given options', searchable=False, clearable=True,
                    ),
                    html.Br(),
                    ...
                    dcc.Dropdown(
                        ...
                        placeholder='Select from the given options', searchable=True, clearable=True,
                    ),
                    html.Br(),
                ...
                html.Div(children=[
                    ...
                    dcc.Input(id='input-lead-time', type='number', placeholder='Enter here',
                              spellCheck=True, required=True),
                    html.Br(),
                    html.Br(),
                    html.Label("Enter any Past Cancellations made by you:"),
                    dcc.Input(id='input-past-cancellations', type='number', placeholder='Enter here',
                              spellCheck=True, required=True)
                ])
            ...
            html.Div(children=[
                ...
                dcc.Loading(
                    id='loading-output', type='cube', color='seagreen',
                    children=[html.Div(id='prediction'), html.Div(id='chances')]
                )
        ...
        return html.Div(children=[
            ...
            html.Div(children=[
                ...
                dcc.Dropdown(id='dropdown-2',
                             ...
                             placeholder='Select from the given options', clearable=True, searchable=False),
            ...
            html.Div(children=[
                html.Div(children=[
                    ...
                    dcc.Checklist(id='checklist-2',
                                  ...
                                  inline=True,
                                  labelStyle={'font-size': 15, 'color': 'black', 'font-family': 'Arial', 'marginBottom': '8px'})
                ], style={'width': '20%', 'minWidth': '200px', 'padding': '15px', 'backgroundColor': 'beige',
                          'borderRadius': '5px', 'marginRight': '20px'}),
                html.Div(children=[
                    dcc.Loading(id='loading-1', type='graph', children=dcc.Graph(id='line-plot')),
                    dcc.Loading(id='loading-2', type='graph', children=dcc.Graph(id='area-plot')),
                    dcc.Loading(id='loading-3', type='graph', children=dcc.Graph(id='bar-plot')),
                    dcc.Loading(id='loading-4', type='graph', children=dcc.Graph(id='hist-plot')),
                    dcc.Loading(id='loading-5', type='graph', children=dcc.Graph(id='pie-plot')),
                    dcc.Loading(id='loading-6', type='graph', children=dcc.Graph(id='sun-plot'))
        ...
        return html.Div(children=[
            ...
            html.Div(children=[
                ...
                html.Div(children=[
                    ...
                    html.Div(children=[
                        ...
                        html.Div(children=[dcc.Graph(figure=comp_fig, config={'responsive': True},
            ...
            html.Div(children=[
                ...
                html.Div(children=[
                    ...
                    html.Div(children=[dcc.Graph(figure=conmat, config={'responsive': True},
                                                  style={'width': '100%', 'height': '300px'})],
                             style={'width': '60%'})
            ...
            html.Div(children=[
                ...
                html.Ol(children=[
                    html.Li("Python"), html.Li("NumPy"), html.Li("Pandas"),
                    html.Li("Plotly Express and Graph Objects"), html.Li("Plotly Dash"), html.Li("Scikit-Learn")
                ])
            ], style={'font-size': 15, 'color': 'black', 'font-family': 'Arial'}),
...
def get_pred(deposit2, country, booking, lead_time, cancellations, button=None):
    if button is None:
        return "", ""
    if deposit2 is None or country is None or booking is None or lead_time is None or cancellations is None:
        ...
        if pred == 0:
            return (html.P(["Guess = ", html.Span("Will Not Cancel !!!", style={'color': 'green'})],
                            style={'font-size': 15, 'color': 'black'}),
                    html.P(["Chances of Guess = ", html.Span(f"{prob[0][0] * 100:.2f}%", style={'color': 'green'})],
                           style={'font-size': 15, 'color': 'black'}))
        else:
            return (html.P(["Guess = ", html.Span("Will Cancel !!!", style={'color': 'red'})],
                            style={'font-size': 15, 'color': 'black'}),
...
def get_graphs(city, hotel, customer=None, deposit=None):
    ...
    if city is None or hotel is None:
        ...
        line_fig = px.line(line, x='No. of Weeks', y='Total Non-Cancellations',
                            title='Trend of Total Non-Cancellations vs No. of Weeks spent by Customers',
                            markers=True, color_discrete_sequence=['brown'])
        ...
        hist_fig = px.histogram(hist, x='Lead Time (in days)', y='Total Non-Cancellations', nbins=10,
        ...
        pie_fig = px.pie(pie, values='Total Cancellations', names='Booking Source',
        ...
        sun_fig = px.sunburst(sun, path=['Country', 'Total Adults', 'Total Children'], values='Total Cancellations',
                               custom_data=['Country', 'Total Adults', 'Total Children'],
...
# NOTE: no `if __name__ == '__main__': app.run(...)` here — Vercel calls
# `server` directly per-request. Use `python api/index.py` locally only if
# you add that block back in temporarily for local testing.
