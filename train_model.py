...
Run this ONCE, locally (not on Vercel), to train the model and cache
everything the Dash app needs. It writes a single file, artifacts.pkl,
which app.py loads at request time instead of retraining from scratch.
...
hotel_data.drop(columns=['arrival_date_year', 'arrival_date_day_of_month', 'children', 'babies',
                          'distribution_channel', 'is_repeated_guest', 'booking_changes', 'agent',
...
hotel_data['Room Category'] = hotel_data['Room Category'].replace(
    ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'],
    ['Standard', 'Economy', 'Family', 'Deluxe', 'Superior', 'Executive', 'Suite', 'Presidential Suite'])
hotel_data['Meal Plan'] = hotel_data['Meal Plan'].replace(
    ['BB', 'HB', 'FB', 'SC'], ['Bed & Breakfast', 'Half Board', 'Full Board', 'Self Catering'])
hotel_data['Booking Source'] = hotel_data['Booking Source'].replace(
    ['Online TA', 'Offline TA/TO', 'Complementary'],
    ['Online Travel Agent', 'Offline Travel Agent/Tour Operator', 'Free Booking'])
hotel_data['Customer Category'] = hotel_data['Customer Category'].replace(
    ['Transient', 'Transient-Party'], ['Individual Booking', 'Individual Booking (for Group)'])
...
# ---------------------------------------------------------------------------
# Modelling — Part 1: Finding the Best Model and Params
# ---------------------------------------------------------------------------
...
for mname, mp in best_model.items():
    clf = RandomizedSearchCV(estimator=mp['model'], param_distributions=mp['params'], n_iter=10, cv=5,
...
# ---------------------------------------------------------------------------
# Modelling — Part 2: Final ML Pipeline
# ---------------------------------------------------------------------------
X = hotel_data.drop(columns=['Cancellation Status'])
Y = hotel_data['Cancellation Status']
...
preprocessor = ColumnTransformer(transformers=[
    ('encoder', OrdinalEncoder(handle_unknown='use_encoded_value', unknown_value=-1,
                                encoded_missing_value=-1, dtype=int), cats_col),
...
# ---------------------------------------------------------------------------
# Persist everything app.py needs — no training logic ships to Vercel
# ---------------------------------------------------------------------------
artifacts = {
    "pipeline": pipeline,          # fitted sklearn Pipeline used for prediction
    "hotel_data": hotel_data,      # cleaned dataframe used by the Dashboard tab
    "score_df": score_df,          # model comparison table/chart data
    "X_columns": list(X.columns),  # column order compute_info2() needs
    "cats_col": cats_col,          # categorical columns list
    "Y_test": Y_test,              # for confusion matrix / metrics table
    "Y_pred": Y_pred,
    "cm": cm,
}
